...
import textwrap
import ConfigParser
import SearchforFile

CFGfile = "./Configfile.yml"
...
def get_settings():
    ...
    if hostfile is None:
        print "We can't read the hostfile from {0}".format(CFGfile)
        sys.exit(2)
...
def example_01(pem_file_path, remote_user, machine_address):
    ...
    "ssh -i {pem_file_path} {remote_user}@{machine_address}".format(
